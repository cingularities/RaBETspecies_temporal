...
setwd("D:/projects/RaBET/NEON_MOAB/HDF5/2017")
...
#imglist <- list("\\\\snow\\2017\\FullSite\\D14\\2017_SRER_3\\L3\\Spectrometer\\Reflectance\\NEON_D14_SRER_DP3_515000_3519000_reflectance.h5")
...
for(f in imglist){
  ...
  reflInfo <- h5readAttributes(f,"/MOAB/Reflectance/Reflectance_Data")
  ...
  for(b in bands) {
    ...
    band <- h5read(f,"/MOAB/Reflectance/Reflectance_Data",index=list(b,1:nCols,1:nRows))
    ...
    mapInfo<-h5read(f,"/MOAB/Reflectance/Metadata/Coordinate_System/Map_Info")
  ...
  # writeRaster(rasterstack,
  #             file= paste(substr(f,1,nchar(f)-3), "bands_all_2017", sep='_'),
  #             format="GTiff",
  #             overwrite=TRUE)
    
  writeRaster(rasterstack,
              file= paste(substr(f,1,nchar(f)-3), "39_bands_nonoise_2017", sep='_'),
              format="GTiff",
              overwrite=TRUE)
...
# 3. define the file name as an object
#imglist <- list("\\\\snow\\2019\\FullSite\\D14\\2019_CLBJ_3\\L3\\Spectrometer\\Reflectance\\NEON_D14_CLBJ_DP3_515000_3519000_reflectance.h5")
...
for(f in imglist){
  # get attributes for the Reflectance dataset
  reflInfo <- h5readAttributes(f,"/CLBJ/Reflectance/Reflectance_Data")
  ...
  for(b in bands) {
    ...
    # Extract or "slice" data for specific band from the HDF5 file
    band <- h5read(f,"/CLBJ/Reflectance/Reflectance_Data",index=list(b,1:nCols,1:nRows))
    ...
    # Populate the raster image extent value. 
    # get the map info, split out elements
    mapInfo<-h5read(f,"/CLBJ/Reflectance/Metadata/Coordinate_System/Map_Info")
  ...
  # writeRaster(rasterstack,
  #             file= paste(substr(f,1,nchar(f)-3), "39_bands_2019", sep='_'),
  #             format="GTiff",
  #             overwrite=TRUE)
  
  writeRaster(rasterstack,
              file= paste(substr(f,1,nchar(f)-3), "39_bands_nonoise_2018", sep='_'),
              format="GTiff",
              overwrite=TRUE)
...
for(f in imglist){
  ...
  writeRaster(rasterstack,
              file= paste(substr(f,1,nchar(f)-3), "39_bands_nonoise_2019", sep='_'),
              format="GTiff",
              overwrite=TRUE)
...
# 3. define the file name as an object
#imglist <- list("\\\\snow\\2021\\FullSite\\D14\\2021_CLBJ_3\\L3\\Spectrometer\\Reflectance\\NEON_D14_CLBJ_DP3_515000_3519000_reflectance.h5")
...
for(f in imglist){
  ...
  # writeRaster(rasterstack,
  #             file= paste(substr(f,1,nchar(f)-3), "39_bands_2021", sep='_'),
  #             format="GTiff",
  #             overwrite=TRUE)
  
  writeRaster(rasterstack,
              file= paste(substr(f,1,nchar(f)-3), "39_bands_nonoise_2020", sep='_'),
              format="GTiff",
              overwrite=TRUE)
