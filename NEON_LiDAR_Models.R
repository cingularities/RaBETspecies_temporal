...
library(MASS)
library(future)
setwd("//snow/projects/RaBET/RaBET_species/NEON_SRER/2021/lidar/")
...
NEONpoints2018 = list.files("//snow/projects/RaBET/RaBET_species/NEON_SRER/2018/lidar",pattern="*.laz$", full.names=TRUE)
...
for(fileName in NEONpoints2021){
#Bring point cloud into Rstudio
#Identify ground points using a cloth simulation filter algorithm
...
#Export the Canopy Height Model to a .tif
writeRaster(NEON_CHM,   
            filename=paste("//snow/projects/RaBET/RaBET_species/NEON_SRER/2021/lidar/",substr(fileName, 76, 89), "_2021_CHM"),
            format="GTiff",
            overwrite=TRUE)
}
...
CHM2021 <- list.files(path = "//snow/projects/RaBET/RaBET_species/NEON_SRER/2021/lidar", pattern = ".tif$", full.names = TRUE)
CHM2021 <- lapply(CHM2021, raster)
CHM2021final <- do.call(merge, c(CHM2021, tolerance = 1))
writeRaster(CHM2021final,
            file= "CHM2021final_SRER",
            format="GTiff",
            overwrite=TRUE)
